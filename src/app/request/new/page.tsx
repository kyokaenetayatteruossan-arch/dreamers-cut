...
export default function NewRequestPage() {
  ...
  const { addJob, uploadFile, sendMessage } = useJobs();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (jobId && referenceUrl) {
        try {
          await sendMessage(jobId, user.id, user.name, "【参考資料を添付しました】", referenceUrl);
        } catch (msgErr) {
          console.error("Auto-message failed:", msgErr);
          // 依頼作成自体は成功しているので、メッセージ送信失敗は致命的でないとする
        }
      }
